...
async function fix() {
  ...
  for (const aksi of perms) {
    const pRes = await query("SELECT id FROM shared.permissions WHERE modul = 'jadwal_pelajaran' AND aksi = $1", [aksi]);
    if (pRes.rows.length > 0) {
       ...
       if (checkRes.rows.length === 0) {
         ...
         console.log('Added permission jadwal_pelajaran.' + aksi + ' to Wakil Kepala');
       }
    }
  }
  process.exit(0);
