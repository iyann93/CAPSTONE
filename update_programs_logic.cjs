...
const mapRegex = /\[\s*\{\s*title:\s*"Beasiswa Prestasi Akademik"[\s\S]*?\]\.map/g;
content = content.replace(mapRegex, 'programList.map');

// 7. Update Delete Action on cards
// Replace `<button className="w-8 h-8 rounded-lg border border-red-100 bg-red-50...`
...
// Wait! I renamed `showAddProgramModal` to `showAddPenerimaModal`
// So it might now be `{/* Add/Edit Program Beasiswa Modal */}` (Wait, I only renamed the variables)
const programModal = `
...
console.log("Applied programmatic updates");
