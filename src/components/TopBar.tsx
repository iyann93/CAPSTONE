...
const UserIcon = () => (
  <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="mr-3">
...
const TopBar: React.FC<TopBarProps> = ({ onToggle, onProfileClick }) => {
  ...
  return (
    <header className="bg-[#1A3D63] text-white h-16 flex items-center justify-between px-6 sticky top-0 z-50 shadow-sm relative">
      {/* Left: Logo & Toggle */}
      <div className="flex items-center gap-3 md:gap-4">
        <button 
          onClick={onToggle}
          className="p-2 hover:bg-white/10 rounded-lg transition-colors md:mr-1"
          aria-label="Menu"
        >
          <MenuIcon />
        </button>
        ...
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center shadow-sm">
            <span className="text-[#1A3D63] font-black text-xl leading-none">S</span>
          </div>
          <span className="font-semibold text-lg tracking-tight hidden sm:block">SIAKAD Terintegrasi</span>
        </div>
      </div>
        ...
        <div className="relative" ref={menuRef}>
          ...
          >
            <div className="hidden md:flex flex-col items-end">
              <span className="text-[13px] font-black leading-tight">Budi Santoso</span>
              <span className="text-[10px] font-bold opacity-60 uppercase tracking-widest mt-0.5">Admin Global</span>
            </div>
            <div className="relative">
              <div className="w-9 h-9 rounded-full bg-white/20 flex items-center justify-center text-white font-black text-xs border border-white/10 shadow-sm overflow-hidden">
                 <img src="https://ui-avatars.com/api/?name=Budi+Santoso&background=4A7FA7&color=fff" alt="Profile" className="w-full h-full object-cover" />
              </div>
            </div>
          ...
          {showProfileMenu && (
            <div className="absolute right-0 mt-3 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-2 z-50 animate-fadeIn">
              <button 
                onClick={() => { setShowProfileMenu(false); if (onProfileClick) onProfileClick(); }} 
              ...
              <button 
                onClick={() => { setShowProfileMenu(false); window.location.href = '/' }} 
