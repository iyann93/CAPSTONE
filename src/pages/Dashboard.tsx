...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  return (
    <main className="flex-1 p-4 md:p-8 flex flex-col gap-6 md:gap-8 overflow-y-auto bg-gray-50/50">
      {/* ── Page Header ── */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 animate-fadeIn">
        ...
        <div className="flex flex-wrap items-center gap-2 md:gap-3">
          ...
          {['Admin', 'Wakil Kepsek'].includes(user.role) && (
            <button className="flex items-center gap-2 bg-primary-500 hover:bg-primary-600 text-white text-xs md:text-sm font-bold px-4 md:px-5 py-2 md:py-2.5 rounded-xl shadow-lg shadow-primary-500/30 transition-all transform hover:scale-[1.02] active:scale-[0.98]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 flex flex-col gap-6">
          <SPPDonutChart />
          {['Admin', 'Kepala Sekolah', 'Guru'].includes(user.role) && (
            <SalaryEstimation />
          )}
        </div>
      </div>
