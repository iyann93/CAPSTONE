  ...
  return (
    <div className="p-6 md:p-8 space-y-6 animate-fadeIn bg-[#F4F6FA] min-h-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        {[
          { label: "Siswa Lunas", val: totalLunas, sub: "dari " + filtered.length + " siswa" },
          { label: "Siswa Menunggak", val: totalTunggak, sub: "perlu ditindaklanjuti" },
          { label: "Total Tunggakan", val: fmt(totalNominal), sub: semester },
        ].map((c, i) => (
          <div key={i} className="bg-white rounded-[24px] p-6 border border-gray-100 shadow-sm">
            <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-2">{c.label}</p>
            <p className="text-[26px] font-black leading-tight text-gray-800">{c.val}</p>
            <p className="text-[12px] mt-1 text-gray-500">{c.sub}</p>
          </div>
        ))}
      </div>
              ...
              {filtered.map(s => (
                <tr key={s.id} className="transition-colors hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3.5">
                    <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider border ${s.status === "Lunas" ? "bg-white text-gray-600 border-gray-200" : "bg-white text-red-500 border-red-100"}`}>
          ...
          <div className="bg-white rounded-2xl w-full max-w-[440px] shadow-2xl">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex justify-between items-center py-4 border-b border-gray-100">
                <span className="text-[12px] font-bold uppercase tracking-wider text-gray-400">Status Pembayaran</span>
                <span className={`text-[14px] font-black uppercase tracking-wider px-3 py-1 border rounded-lg ${detail.status === "Lunas" ? "text-gray-600 border-gray-200 bg-gray-50" : "text-red-500 border-red-100 bg-white"}`}>
              ...
              {detail.status === "Tunggak" && (
                <div className="bg-white border border-gray-200 rounded-xl p-4 text-[12px] text-gray-500 text-center">
                  Pembayaran SPP perlu segera ditindaklanjuti.
                </div>
              )}
            </div>
