...
const FileTextIcon = () => (
  <svg width="48" height="48" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24" className="text-gray-200 mb-4">
    <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M16 13H8"/><path d="M16 17H8"/><path d="M10 9H8"/>
  </svg>
)

const ERapor: React.FC = () => {
  const students = [
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-gray-50">
            {students.map((s) => (
              <div key={s.id} className="p-5 flex items-center justify-between hover:bg-gray-50/50 transition-colors">
                ...
                <div className="flex items-center gap-10">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-white text-[10px] font-black w-5 h-5 flex items-center justify-center rounded-full shadow-sm transition-colors ${
                      s.status === 'Final' ? 'bg-[#5EE9B5]' : 'bg-[#4A7FA7]'
                    }`}>
                      {s.grade}
                    </span>
                  </div>
                  
                  <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider transition-all ${
                    s.status === 'Final' 
                      ? 'bg-[#5EE9B5] text-white shadow-sm' 
