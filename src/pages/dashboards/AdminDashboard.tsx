...
interface DashboardProps { user: User; activeMenu: string }
...
const BarChart = () => {
  const bars = [
    { h: 72, label: 'Kls 10' }, { h: 85, label: 'Kls 11' }, { h: 60, label: 'Kls 12' },
    { h: 90, label: 'Kls 10' }, { h: 75, label: 'Kls 11' }, { h: 82, label: 'Kls 12' },
  ]
  return (
        ...
        <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', height: '100%' }}>
          <div style={{ width: '100%', flex: 1, backgroundColor: '#F3F4F6', borderRadius: '6px 6px 0 0', position: 'relative', overflow: 'hidden' }}>
            <div style={{
              ...
              height: `${b.h}%`, backgroundColor: '#1A3D63',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <div style={{ position: 'relative', width: 140, height: 140 }}>
        <svg width={140} height={140} viewBox="0 0 140 140">
          ...
          <circle cx={cx} cy={cy} r={r} fill="none" stroke="#5EE9B5" strokeWidth={16}
            strokeLinecap="round"
...
const DashboardOverview: React.FC = () => {
  const card = (iconBg: string, iconColor: string, icon: React.ReactNode, label: string, value: string, sub: string) => (
    <div className="bg-white rounded-xl border border-gray-100 p-4 sm:p-5 flex items-center gap-3 sm:gap-4 shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 sm:p-6 shadow-sm">
          ...
          <div className="flex flex-col gap-3">
            {scheduleItems.map((j, i) => (
              <div key={i} className="flex items-start sm:items-center gap-3 sm:gap-4 bg-gray-50 rounded-lg p-3 sm:p-4 border-l-4 border-[#1A3D63]">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-50">
                {['WAKTU','USER','MODUL','AKTIVITAS'].map(h => (
                  <th key={h} className="px-6 py-2.5 text-left text-[11px] font-semibold text-gray-400 tracking-wide">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
...
// ── Main Component ───────────────────────────────────────────
const AdminDashboard: React.FC<DashboardProps> = ({ user, activeMenu }) => {
  const content = () => {
    switch (activeMenu) {
      case 'Kelola Pengguna':         return <ManageUsers />
      case 'Data Siswa':              return <StudentData />
      case 'Data Guru & Karyawan':    return <EmployeeData />
      case 'Tahun Ajaran & Semester': return <AcademicYear />
      case 'Mata Pelajaran':          return <Subjects />
      case 'Kelas & Penjadwalan':     return <Classes />
      case 'E-Rapor & Input Nilai':   return <ERapor />
      case 'Laporan Integrasi':       return <FinancialReport />
      case 'Pengaturan Sistem':       return <SystemSettings />
      case 'My Profile':              return <Profile user={user} />
      default:                        return <DashboardOverview />
    }
  }
  return (
    <main className="flex-1 p-4 sm:p-6 overflow-y-auto bg-[#F3F4F6] min-h-screen">
      {content()}
    </main>
  )
}
