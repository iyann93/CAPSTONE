...
const mockSubjects = [
  ...
  { mapel: "Bahasa Indonesia", guru: "Ibu Sari", nilai: [88, 90, 85, 92], kkmPassed: true },
  { mapel: "Bahasa Inggris", guru: "Ibu Lena", nilai: [76, 80, 74, 82], kkmPassed: true },
  { mapel: "Fisika", guru: "Bpk. Agus", nilai: [70, 68, 75, 72], kkmPassed: true },
  { mapel: "Kimia", guru: "Ibu Rina", nilai: [85, 88, 90, 87], kkmPassed: true },
  { mapel: "Biologi", guru: "Ibu Dewi", nilai: [90, 92, 88, 95], kkmPassed: true },
  { mapel: "Sejarah", guru: "Bpk. Joko", nilai: [78, 80, 76, 84], kkmPassed: true },
...
const AkademikSiswa = () => {
  ...
  const allAvg = parseFloat(getAvg(mockSubjects.map(s => parseFloat(getAvg(s.nilai)))));
  const best = [...mockSubjects].sort((a, b) => parseFloat(getAvg(b.nilai)) - parseFloat(getAvg(a.nilai)))[0];
  const lowest = [...mockSubjects].sort((a, b) => parseFloat(getAvg(a.nilai)) - parseFloat(getAvg(b.nilai)))[0];

  const barLabels = ["UH 1", "UH 2", "UTS", "UAS"];
    ...
    <div className="p-6 md:p-8 space-y-6 animate-fadeIn bg-[#F4F6FA] min-h-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: "Mapel Terbaik", val: best.mapel, sub: `Avg: ${getAvg(best.nilai)}`, color: "text-green-600", bg: "bg-green-50" },
          { label: "Perlu Perhatian", val: lowest.mapel, sub: `Avg: ${getAvg(lowest.nilai)}`, color: "text-amber-600", bg: "bg-amber-50" },
          { label: "Total Mata Pelajaran", val: mockSubjects.length, sub: "Semua lulus KKM", color: "text-purple-600", bg: "bg-purple-50" },
        ].map((card, i) => (
          <div key={i} className="bg-[#1A3D63] rounded-2xl p-5 shadow-sm">
            ...
            <p className="text-[24px] font-black text-white leading-tight">{card.val}</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="border-b border-gray-100">
                <tr>
                  {["MATA PELAJARAN", "GURU", "UH 1", "UH 2", "UTS", "UAS", "RATA-RATA", "STATUS"].map(h => (
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="flex items-end gap-2 h-28 mt-2">
              {(selectedMapel ? selectedMapel.nilai : mockSubjects.map(s => parseFloat(getAvg(s.nilai)))).slice(0, 8).map((v, i) => {
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    ...
                    <span className="text-[9px] text-gray-400 truncate w-full text-center">
                      {selectedMapel ? barLabels[i] : (mockSubjects[i]?.mapel?.slice(0, 4) || "")}
                    </span>
                  </div>
                );
              })}
          ...
          {selectedMapel && (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 animate-fadeIn">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-[14px] font-bold text-gray-700">{selectedMapel.mapel}</h3>
                <button onClick={() => setSelectedMapel(null)} className="text-gray-400 hover:text-gray-600">
              ...
              <p className="text-[12px] text-gray-400 mb-3">Guru: {selectedMapel.guru}</p>
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4">
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-[12px] text-blue-600 mt-1 leading-relaxed">Ahmad menunjukkan peningkatan di semester ini. Terus dukung belajar di rumah!</p>
              </div>
            </div>
          </div>
        </div>
