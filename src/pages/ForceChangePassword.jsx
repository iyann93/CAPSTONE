...
const ForceChangePassword = ({ onSuccess, onLogout }) => {
  const [password, setPassword] = useState("");
  const [confirmPassword, setConfirmPassword] = useState("");
  const [strength, setStrength] = useState(0);
  const [strengthText, setStrengthText] = useState("Lemah");
  ...
  useEffect(() => {
    ...
    if (password.length === 0) {
      setStrengthText("");
      setStrengthColor("text-gray-400");
    } else if (score < 3) {
      setStrengthText("Lemah");
      setStrengthColor("text-red-500");
    } else if (score < 5) {
      setStrengthText("Sedang");
      setStrengthColor("text-amber-500");
    } else {
      setStrengthText("Kuat");
  ...
  const handleSave = async () => {
    ...
    try {
      // Endpoint require JWT auth
      const res = await api.post("/auth/change-password", { newPassword: password });
      if (res.data.success) {
      ...
      console.error(e);
      alert(e.response?.data?.message || "Terjadi kesalahan saat menyimpan password.");
    } finally {
  ...
  />
            ...
            <div className="pt-2 pb-4">
              <div className="flex flex-wrap justify-between items-center mb-2">
                ...
                <span className={`text-[10px] font-bold uppercase tracking-widest ${strengthColor}`}>{strengthText}</span>
              </div>
              <div className="flex gap-1.5 mb-2">
                <div className={`h-1.5 flex-1 rounded-full ${strength >= 1 ? (strength < 3 ? 'bg-red-500' : strength < 5 ? 'bg-amber-500' : 'bg-green-500') : 'bg-gray-200'}`} />
                <div className={`h-1.5 flex-1 rounded-full ${strength >= 2 ? (strength < 3 ? 'bg-red-500' : strength < 5 ? 'bg-amber-500' : 'bg-green-500') : 'bg-gray-200'}`} />
                <div className={`h-1.5 flex-1 rounded-full ${strength >= 3 ? (strength < 5 ? 'bg-amber-500' : 'bg-green-500') : 'bg-gray-200'}`} />
                <div className={`h-1.5 flex-1 rounded-full ${strength >= 4 ? 'bg-green-500' : 'bg-gray-200'}`} />
              </div>
    ...
    className="w-full bg-[#1A3D63] hover:bg-[#122A44] disabled:opacity-50 text-white font-bold py-3.5 rounded-[16px] transition-all shadow-[0_10px_30px_-10px_rgba(26,61,99,0.3)] active:scale-[0.98] mt-2 flex items-center justify-center text-[14px]"
  >
              <SaveIcon />
    ...
    /* Bottom Actions */
        ...
        <div className="w-full mx-auto">
          ...
          <div className="flex flex-wrap items-center justify-between">
            <span className="text-gray-400 text-[12px]">Bukan akun Anda?</span>
            <button
    onClick={onLogout}
