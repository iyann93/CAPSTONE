...
const EyeIcon = ({ show }: { show: boolean }) => (
  <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="text-gray-400 hover:text-gray-600 transition-colors">
    {show ? (
      <>
...
const NewsCard = ({ category, categoryColor, title, description, time }: any) => (
  <div className="bg-[#2A4B6D] border border-[#37587E] rounded-[20px] p-6 mb-4 relative overflow-hidden group hover:bg-[#2E5175] transition-all duration-300">
...
const Login: React.FC<LoginProps> = ({ onLogin, onForgotPassword }) => {
  ...
  return (
    <div className="flex flex-col-reverse lg:flex-row min-h-screen w-full bg-white font-sans relative overflow-x-hidden overflow-y-auto">
      ...
      <div className="w-full lg:w-[40%] min-h-[100dvh] lg:min-h-screen bg-white flex flex-col p-6 sm:p-8 lg:px-20 xl:px-24 py-10 lg:py-16 justify-between relative z-10">
        ...
        <div className="max-w-[400px] w-full mx-auto relative z-10 my-auto pt-12 lg:pt-16 pb-12">
          ...
          <div className="space-y-8">
            {error && (
              <div className="bg-amber-500/10 border border-amber-500/20 text-amber-600 text-xs py-3.5 px-4 rounded-xl font-medium flex items-center gap-2">
